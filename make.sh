...
while getopts 'lwdva' flag; do
  case "$flag" in
    ...
    w) OS="windows"
       ...
       ISPECIFIC="-I lib/vlc-$OS/include"
       LSPECIFIC="-L lib/vlc-$OS/lib -lopengl32";;
    d) DEBUG="-g3"
       O="-O0";;
    v) VALGRIND="true"
       DEBUG="-g3"
...
L="-lvlc -L lib/SFML-2.5.1-$OS/lib -lsfml-graphics -lsfml-audio -lsfml-window -lsfml-system"
ISFML="-I lib/SFML-2.5.1-$OS/include -I lib/imgui -I lib/imgui-sfml"

log "Building for $RED$OS$NC..."

$CC $O $CFLAGS -o obj/$OS-service-vlc.o -c src/services/service_vlc.cpp $ISFML $ISPECIFIC -pthread 
$CC $O $CFLAGS -o obj/$OS-service-script.o -c src/services/service_script.cpp $ISFML $ISPECIFIC -pthread 
$CC $O $CFLAGS -o obj/$OS-locator.o -c src/services/locator.cpp $ISFML $ISPECIFIC -pthread 
